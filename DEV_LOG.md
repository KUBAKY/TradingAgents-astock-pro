...
**案例文件归档:**
- 已于 2026-07-24 移除。本项目不在仓库内提供针对具体证券的分析报告或评级结论;
  `examples/run_cases.py` 保留为可自行运行的技术演示脚本,运行结果由使用者自行保管。
...
### 阶段进度（P1~P6 + ①~⑥ 对应 commit 从 `205b17a` 起共 25 个；⑦⑧ 已提交 `8b188c2`/`fdfcdc0`/`961a05b`）
...
- **⑨ 选股增强八件套（P1~P8，TDD）**：①板块梯队 `assign_tiers`（screener 按 industry 分组：龙头=第一个连板票、中位股=连板≥2 非前三「容易A杀回避」、低位补涨=无连板+7日>5%+龙头已涨停、跟风杂毛，无涨停时 activity 第一标注「缺涨停确认」）+ SCREENER_PROMPT 梯队纪律（中位股/跟风禁入操作计划）；②情绪周期温度计 `fetch_market_sentiment`（push2ex getTopicZTPool 涨停家数+最高连板 lbc + clist 跌幅榜 top100 近似跌停，五档：高潮>100/冰点<20且streak<3/退潮<20且streak≥3 或跌停≥20且涨停<40/修复20-39/升温40-100，失败降级 unknown 不阻塞）；③量价形态五信号 `detect_volume_price_patterns`（放量突破/缩量回调/价量背离/放量滞涨/强封板，挂 metrics["patterns"] + ch0 摘要渲染）；④融资风险线 `get_margin_data`（东财 RPTA_RZRQ_GGMX，两融余额/流通市值 >15% → margin_risk 异常）；⑤主题生命周期 prompt 注入（`_theme_stats` 概念聚合 TOP5 + SCREENER_PROMPT 步骤5 生命周期判断：兴起/扩散参与、过热仅龙头低吸、兑现/退潮禁止）；⑥龙虎榜散户席位识别 `get_lhb_seats` + `detect_retail_takeover`（拉萨/东环路/团结路名单，买方散户净买>卖方且非散户卖出>0 → retail_takeover 异常）；⑦机构行为反向推理 `_institutional_activity`（OPERATEDEPT_CODE="0" 机构专用，净买滞涨→建仓中/净卖放量→出货中，ch0 摘要「机构动向」渲染）；⑧决策卡 horizon 字段（适用周期 schema 校验 + parse_decision 解析已覆盖）；**实测**：08-04 情绪 138 涨停/7 板→高潮、603256 机构净卖 5.64 亿识别正确；测试 126 相关用例 + 全量回归仅剩 pre-existing agent_sdk 2 失败（claude_agent_sdk Server.list_tools API 变更，与本次无关）
- **⑩ 短线↔主线四项融合（#1~#4，TDD）**：**①盘面信号注入主线 analyst**（`shortterm/pulse.py::build_market_pulse` — ch0 异动精扫 + 情绪温度计格式化为 ~20 行文本，零 LLM 成本；`propagation.create_initial_state` 算好注入 `state["market_pulse"]`（lazy import 防 graph→shortterm 循环）；`AgentState` 加字段；social/policy analyst system_message 加「📡 短线盘面信号参考」段落，policy 附加政策-异动交叉验证指令；任何失败降级空串不阻塞主线；实测 08-04 情绪 138 涨停/7 板→高潮正常注入）；**③决策卡基本面快查**（`gather_data_bundle` 加「基本面快查」section 用现成 `get_fundamentals`（腾讯估值+mootdx 财报+东财信息+同花顺 EPS 一致预期，零额外 LLM 调用）；ULTRA_SHORT/SWING 模板加纪律7「基本面雷区检查」+ 决策卡新字段「基本面雷区检查」（商誉/质押/解禁/业绩变脸/退市，有雷默认回避/观望，可一票否决）；实测 603256 PE 364/PB 42 高估值清晰入包）；**共通件 `shortterm/deep_review.py`**（`run_deep_review` — lazy import `TradingAgentsGraph`、`DEFAULT_CONFIG` 默认、`graph.propagate` 全链路（15-20min/票）→ 落盘 `~/.tradingagents/shortterm/deepreview/`（ticker_date_ms.json，含 5 档评级 + 11 份 analyst 报告 + 触发原因），毫秒 ts 防同秒乱序；异常捕获返回 error 不抛）；**②持仓异常→主线复核**（`portfolio.run_daily_follow`：决策卡方向=卖出 或 ch0 高危异动（retail_takeover/margin_risk/overheat）→ 标记 `entry.deep_review` + 跟进后串行 `run_deep_review`（`max_deep_reviews=2` 票/日防成本爆炸，可配），entry 落 `deep_review_signal/deep_review_path/deep_review_error`；持仓页跟进结果表 + 历史快照表加「深度复核/复核报告」列 + 触发 info 提示）；**④扫描 TOP3→主线深度辩论**（`screener.extract_picks` — 正则提主流 A 股代码前缀（00/30/60/68/8x/43/92）按出现顺序去重，负前瞻排除价格（300750.5）等假阳性；短线页扫描 done 分支加「对推荐 TOP 3 跑深度辩论」按钮 → `start_job("deep_review")` 后台串行 3×run_deep_review（每票 15-20 分钟，独立 job 轮询 30s + 结果表展示 评级/报告路径））；**测试**：新增 4 测试文件 42 用例（pulse 格式化/降级/截断、propagation 注入、analyst prompt 渲染、bundle 基本面 section、雷区指令、deep_review 落盘/失败/load、extract_picks 解析/去重/假阳性）+ portfolio 深度复核触发/上限/失败 11 用例；全量 461 passed/1 skipped，仅剩 pre-existing agent_sdk（已 34 例 deselect）；**成本注记**：#1 单票 +30-65s 数据请求无 LLM（mootdx 熔断时 sina 兜底慢）；#3 +2-4k token 无额外调用；#2 触发才跑主线上限 2/日；#4 手动 3×主线 45-60min
- **⑪ 统一分析结果关联保存 + 自动对比 + 结果验证 + 验证反馈闭环（6 步，TDD）**：**①注册表 `analysis_registry/registry.py`** — 六类结果统一索引（KINDS=stock/follow/screen/pick/deep_review/mainline），record_id=`<kind>:<ticker>:<trade_date>:<ts>` 幂等注册，目录 `~/.tradingagents/analysis_registry/`（`TRADINGAGENTS_REGISTRY_DIR` 覆盖）：`index.json` + `compare/` + `validate/` + `reports/`；增量注册 + **首次 backfill**（真实历史 17 条导入，stock 3/screen 9/pick 5）；4 个注册点 wrapper 全接：短线 pipeline/screener/portfolio 跟进、deep_review 落盘；**②自动对比 `compare.py`** — `after_register` 同票最近两条摘要规则对比（方向/置信度/目标价/止损/周期/模式/异动 diff，delta 百分比）→ `compare/<ticker>_<id>__vs__<id>.json` 幂等落盘；`build_llm_compare` 深度对比仅手动触发（LLM 读原文截断 6000 字符 + 成本记账，双记录 id + 省略字段防重复提交）；**③盘后验证 `validate.py`** — 窗口：stock/follow=3 交易日、其余=10；幂等三态：已有定论且 asof≥本次跳过 / 「待验证」允许重算 / force 强制；rating→方向映射（Buy/Overweight→买入、Hold→观望、Underweight/Sell→卖出）；基准收盘价缺省从 K 线 `_entry_close_from_df` 补齐（内存副本不回写原文）；单条异常降级「验证失败」绝不抛；`scripts/run_validations.py` CLI + launchd `com.tradingagents.validate`（工作日 15:35 已装，日志 `~/.tradingagents/logs/validate.log`，与 close-scan/close-portfolio 并存）；**④验证反馈 `feedback.py`** — `scored_records` 纯读索引 validation 字段零重算；`build_feedback_report` 按方向/置信度/模式/异动类型聚合错判率 → `reports/feedback_<asof>.md`；`feedback_injection_block` 注入短线 pipeline v2.1「误差反馈」段（同方向错判率 ≥50% → 「为何这次不同」纪律块，prompt 零变化兜底失败空串）；**⑤UI `web/pages/3_分析对比.py`** — 同票跨方式时间线（含验证列）/ 规则对比报告折叠 / 手动 LLM 深度对比（成本显示）/ 手动补验证 / 误差报告生成五区块；1_短线分析.py 复盘 tab 加 page_link 入口；**测试**：registry 21 + compare 17 + validate 21 + feedback 10 = 69 用例，conftest autouse 四目录（registry/shortterm/logs/deepreview）tmp 隔离；**实测**：600519 三条 follow 验证正确（观望→不评分、卖出→K 线不足→待验证）、模拟新注册自动出对比报告（同摘要 changes 空仍落盘可追溯）、0 评分记录误差报告正常生成、600519 注入块为空（正确）；全量 562 passed/1 skipped/45 subtests，仅剩 pre-existing agent_sdk 2 失败（同 ⑩）
...
- 涨停池 getTopicZTPool 参数必须 `sort=fbt:asc`（首板时间），否则东财默认排序连板票不一定在头部；`lbc`=连板数字段
- 东财 datacenter `filter` 拼 `(TRADE_DATE<='None')` 不报错但返回空 → `get_lhb_seats` trade_date=None 时须省略 filter（本轮修复 + 回归测试）
- 测试污染：screener.recommend 内 `cost_context("screener")` 不 reset → 后续 test_cost 断言默认 feature 失败；prompt 测试 monkeypatch `_cost` 假对象隔离
- 对比测试夹具坑：`_rec` 默认参数吞 `path=`（签名无该参数静默接受？不，是默认按 position 传错位）→ 两条记录 path 相同导致「无可读原文」；改显式 `path=` 关键字传参 + 真实 tmp 文件后稳定
- conftest 隔离必须覆盖**数据源目录**（SHORTTERM/LOGS/DEEPREVIEW env），否则测试写进真实 `~/.tradingagents/`（compare/feedback 测试最先踩中）；registry autouse 后四目录全 tmp
- test_feedback 断言算错：2/2 错判率应为 100.0% 非 50.0% — 测试修对，实现本就正确
- 真实数据 0 份对比报告是**设计如此**：backfill 不触发 after_register（仅增量注册触发），之后无新注册自然无报告；模拟新注册验证链路后正常出报告
- mcp 2.0.0 移除 `Server.list_tools` → claude_agent_sdk 0.2.95 `_bind_tools` AttributeError（pre-existing，非本次）；deselect 或升 claude-agent-sdk 待窗口
