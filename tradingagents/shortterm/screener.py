...
_FIELDS = "f2,f3,f5,f6,f8,f10,f12,f14,f20,f21"


def fetch_board_snapshot(board: str, top: int = 80) -> list[dict]:
    """东财push2 clist：单请求拿整板快照，按成交额排序。"""
...
def scan(capital: float | None = None, per_board: int = 8,
    ...
    result = {"trade_date": trade_date, "capital": capital, "boards": {}}

    for board in BOARD_FS:
        snap = activity_rank(fetch_board_snapshot(board))
        candidates = []
        for row in snap[:per_board]:
            try:
                ch0 = run_ch0(row["code"], trade_date)
            except Exception as e:
                row["ch0_error"] = str(e)[:80]
...
SCREENER_PROMPT = """你是A股短线选股顾问。以下是程序化三级扫描后的候选池（每板块按活跃度粗排后，已过黑名单/异动精扫）。用户资金 {capital} 元。

任务：沪深主板、创业板、科创板各选出不超过3只"活跃且值得操作"的标的，给出：
1. **为什么选它**（结合异动信号/题材/资金，不用泛泛而谈）
2. **操作模式**（隔日超短 / 3-10日波段）与**介入方式**（追/等回踩/等竞价确认）
3. **资金分配**：总资金如何在选中标的间分配（单票仓位上限：超短15%，波段30%）
4. **排雷说明**：候选池里你主动剔除的票及原因（过热/分歧过大/数据缺失）
...
def recommend(scan_result: dict, provider: str, model: str,
    ...
    slim = {"trade_date": scan_result["trade_date"], "boards": {}}
    for board, rows in scan_result["boards"].items():
        slim["boards"][board] = rows[:6]
