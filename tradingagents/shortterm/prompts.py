...
### 基本面雷区检查
（对照"基本面快查"数据逐项检查：高商誉减值风险/大股东高比例质押/近期大额解禁/业绩变脸/退市风险。无雷写"未发现明显雷区"；有雷则结合 方向 判断是否一票否决）
"""
...
7. **基本面雷区检查**: 对照数据包「基本面快查」检查商誉/质押/解禁/业绩变脸/退市风险。短线也要避开基本面雷——一旦踩雷跌停连跑的机会都没有。有雷则方向默认 回避 或 观望，并在决策卡「基本面雷区检查」注明。
...
7. **基本面雷区检查**: 对照数据包「基本面快查」检查商誉/质押/解禁/业绩变脸/退市风险。波段持仓周期 3-10 日，基本面雷（尤其解禁/减持/业绩变脸）会直接压垮波段逻辑。有雷则方向默认 回避 或 观望，并在决策卡「基本面雷区检查」注明。
...
def ch0_summary_block(ch0: dict) -> str:
    ...
    if pats:
        lines.append("- 量价形态信号:")
        for p in pats:
            lines.append(f"  * [{p['type']}] {p['signal']}")
    ...
    if inst:
        lines.append(
            f"- 机构动向(龙虎榜): 买入 {inst['buy_yi']}亿 / 卖出 {inst['sell_yi']}亿"
            f" / 净额 {inst['net_yi']}亿（反向推理：机构净买但股价滞涨→建仓中；"
            "机构净卖且放量→出货中，据此解读当日走势）")
